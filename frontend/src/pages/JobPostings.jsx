...
export default function JobPostings() {
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    {tab === 'list' ? <>
      ...
      <section className="match-job-band"><div><p className="job-label">Eşleştirilecek iş ilanı</p><select className="job-input min-w-[320px]" value={selectedJobId} onChange={event => { setSelectedJobId(event.target.value); setMatchResponse(null); }}><option value="">İlan seçin</option>{jobs.map(job => <option key={job.id} value={job.id}>{job.title} · {job.company_name}</option>)}</select></div>{selectedJob && <div className="min-w-0 flex-1 border-l border-surface-200 pl-6"><p className="font-bold text-gray-900">{selectedJob.title}</p><p className="mt-1 text-sm text-gray-500">{selectedJob.company_name} · {(selectedJob.required_skills || []).length} zorunlu yetkinlik</p></div>}<button className="rounded-lg border border-primary-200 px-4 py-2.5 text-sm font-semibold text-primary-600 hover:bg-primary-50 disabled:opacity-50" onClick={matchExisting} disabled={!selectedJobId || matchingExisting}><Sparkles size={16} className="mr-2 inline" />{matchingExisting ? 'Eşleştiriliyor' : 'Kayıtlı adayları eşleştir'}</button></section>
